...
const defu = createDefu((obj, key, value, namespace) => {
    ...
    if (namespace === 'resources.Resources' && Object.hasOwn(obj, key)) {
        throw new ServerlessError(
            `'resources.Resources.${String(key)}' is already defined.`,
...
export class Module {
    ...
    static initialize(serverless: Serverless) {
        ...
        for (const [id, construct] of Object.entries(config.constructs)) {
            ...
            for (const source of [config, moduleConfig, overrides]) {
                if (isRecord(source.provider)) {
                    if ('iamManagedPolicies' in source.provider) {
                        throw new ServerlessError(
                            "'provider.iamManagedPolicies' is not supported. Use 'provider.iam.role.managedPolicies' instead.",
                            'LIFT_INVALID_CONSTRUCT_CONFIGURATION',
                        );
                    }

                    if ('iamRoleStatements' in source.provider) {
                        throw new ServerlessError(
                            "'provider.iamRoleStatements' is not supported. Use 'provider.iam.role.statements' instead.",
                            'LIFT_INVALID_CONSTRUCT_CONFIGURATION',
                        );
                    }
                }
                ...
                if (Array.isArray(source.resources)) {
                    throw new ServerlessError(
                        "The array form of 'resources' is not supported. Use the object form instead.",
                        'LIFT_INVALID_CONSTRUCT_CONFIGURATION',
                    );
                }

                if (source !== config && 'constructs' in source) {
                    throw new ServerlessError(
                        "'constructs' is not supported in modules.",
                        'LIFT_INVALID_CONSTRUCT_CONFIGURATION',
                    );
                }
            }
